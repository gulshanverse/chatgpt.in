...
import { cancelChatFileUpload, queueChatFileUploads } from "../../lib/chat/upload-store";
...
export default function ChatPage() {
  ...
  function addFiles(files: FileList | null) {
    if (!files) return;
    const selected = Array.from(files).map((file) => ({ file, clientId: crypto.randomUUID() }));
    setAttachmentError(null);
    setAttachments((current) => [...current, ...selected.map(({ file, clientId }) => ({ id: clientId, name: file.name, type: file.type || "application/octet-stream", size: file.size }))]);
    void queueChatFileUploads(selected).catch((uploadError) => setAttachmentError(uploadError instanceof Error ? uploadError.message : "Unable to upload attachment"));
  }
  ...
  async function runGeneration(conversationId: string, history: ChatMessage[], content: string, selectedAttachments: ChatAttachment[] = []) { setAssistantText(""); let full = ""; try { await send({ conversationId, content, history: history.map(({ role, content: messageContent }) => ({ role, content: messageContent })), model, attachments: selectedAttachments, onToken: (token) => { full += token; setAssistantText(full); } }); if (full) addMessage(conversationId, { role: "assistant", content: full }); setAssistantText(""); return true; } catch { return false; } }
  async function submit(event?: FormEvent) { event?.preventDefault(); if (isStreaming) { stop(); return; } if (recording || transcribing) return; const selectedAttachments = attachments; const text = draft.trim() || (selectedAttachments.length ? `Please analyze these attachments: ${selectedAttachments.map((file) => file.name).join(", ")}` : ""); if (!text) return; setAttachmentError(null); const conversation = active ?? createConversation(text.slice(0, 48)); const userMessage: ChatMessage = { id: crypto.randomUUID(), role: "user", content: text, createdAt: new Date().toISOString(), attachments: selectedAttachments }; setActiveId(conversation.id); addMessage(conversation.id, { role: "user", content: text, attachments: selectedAttachments }); setDraft(""); const succeeded = await runGeneration(conversation.id, [...conversation.messages, userMessage], text, selectedAttachments); if (succeeded) setAttachments([]); }
  async function regenerate() { if (!active || isStreaming || recording || transcribing) return; const lastUser = [...active.messages].reverse().find((message) => message.role === "user"); if (!lastUser) return; const userIndex = active.messages.findIndex((message) => message.id === lastUser.id); const preserved = active.messages.slice(0, userIndex + 1); replaceMessages(active.id, preserved); await runGeneration(active.id, preserved, lastUser.content, lastUser.attachments ?? []); }
